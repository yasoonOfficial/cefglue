...
class obj_function:
    ...
    def get_cefglue_parts(self, defined_structs = [], prefix = None):
        ...
        retval = ''
        dict = self.retval.get_type().get_cefglue(defined_structs)
        if dict['format'] == 'single':
            retval = dict['value']
        ...
        if isinstance(self, obj_function_virtual):
            # virtual functions get themselves as the first argument
            str = self.parent.get_capi_name()+'* self'
            if isinstance(self, obj_function_virtual) and self.is_const():
                # const virtual functions get const self pointers
                str = '/*const*/ ' + str
            args.append(str)
        ...
        if len(self.arguments) > 0:
            for cls in self.arguments:
                type = cls.get_type()
                dict = type.get_cefglue(defined_structs)
                if dict['format'] == 'single':
                    args.append(dict['value'])
                elif dict['format'] == 'multi-arg':
                    # add an additional argument for the size of the array
    ...
    def get_cefglue_proto(self, defined_structs = [], prefix = None):
...
class obj_analysis:
    ...
    def get_cefglue(self, defined_structs = []):
        """ Format the value for the C API. """
        result = ''
        format = 'single'
        ...
        if self.has_name() and self.get_name() == 'dirtyRects':
            ...
            result += self.get_cefglue_result_struct_type(defined_structs)
        elif self.is_result_string():
            ...
            resdict = self.get_cefglue_result_vector_type(defined_structs)
            if resdict['format'] != 'single':
                format = resdict['format']
            result += resdict['value']
        
        if self.has_name() and format != 'multi-func':
            result += ' ' + self.get_cefglue_name();
        
        return {'format' : format, 'value' : result}

    def get_cefglue_name(self):
        name = self.get_name()
        if name == 'string' or name == 'object' or name == 'event':
            name = '@' + name
        return name
    ...
    def get_cefglue_result_simple_type(self):
        ...
        if self.is_const():
            result += '/*const*/ '
        result += self.get_cefglue_type( self.result_value )
        if self.is_byaddr() or self.is_byref():
            result += '*'
        return result
    
    def get_cefglue_result_refptr_type(self, defined_structs = []):
        """ Return the refptr type. """
        result = ''
        if not self.result_value[:-1] in defined_structs:
            result += ''
    ...
    def get_cefglue_result_struct_type(self, defined_structs = []):
        """ Return the structure or enumeration type. """
        result = ''
        # structure values that are passed by reference or address must be
        # structures and not enumerations
    ...
    def get_cefglue_result_string_type(self):
        """ Return the string type. """
        if not self.has_name():
            # Return values are string structs that the user must free. Use
            # the name of the structure as a hint.
    ...
    def get_cefglue_result_vector_type(self, defined_structs = []):
        """ Return the vector type. """
        if not self.has_name():
            raise Exception('Cannot use vector as a return type')
